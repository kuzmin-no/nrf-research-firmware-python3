...
with open(sys.argv[1]) as f:
  ...
  payloads[0] = payloads[0][0:2] + (payloads[0][2] + 1).to_bytes(1, "little") + (payloads[0][3] - 1).to_bytes(1, "little") + payloads[0][5:]
...
for x in range(0, 0x70, 2):
  response = dongle.send_command(0x21, 0x09, 0x0200, 0x0000, b"\x30" + (x).to_bytes(1, "little") + b"\x00\x01" + b"\x00"*28)
